  ...
  return new Promise((resolve, reject) => {
    jwt.sign(
      tokenData, JWT_SECRET, signOptions, (err, token) => {
        if (err) {
          console.log('Error tijdens tekenen van token:', err.message);
          return reject(err);
        }
  ...
  return new Promise((resolve, reject) => {
    jwt.verify(
      authToken, JWT_SECRET, verifyOptions, (err, decodedToken) => {
        if (err || !decodedToken) {
          console.log('Error tijdens verificatie van token:', err.message);
          return reject(err || ServiceError.unauthorized('Token kan niet geparsed worden'));
        }
